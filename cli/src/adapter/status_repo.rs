...
mod tests {
    ...
    fn test_is_referenced_by_tasks() {
        ...
        .unwrap();
        conn.execute(
            "INSERT INTO tasks (id, title, description, label, status_id, workspace_id, created_at, updated_at) VALUES ('t1', 'task', 'desc', 'feature', '00000000000000000000000000000001', 'p1', '2026-01-01T00:00:00+00:00', '2026-01-01T00:00:00+00:00')",
            [],
        )
        .unwrap();
