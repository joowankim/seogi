...
│       ├── log_reader.rs     # JSONL 파싱 (pretty-printed + compact)
│       ├── models.rs         # LogEntry, SessionMetrics 구조체
│       ├── analyzers/
│       │   ├── mod.rs
...
- `config.rs` — config.json 로드, ~/seogi-logs 경로 해석

### Step 2: JSONL 파서 (log_reader.rs)
...
- 기존 분석기와 동일한 metrics JSONL 스키마 출력 확인

### Step 5: `seogi report` 명령어

- metrics JSONL 읽기 + 날짜 범위 필터
...
- `cargo build --release` 후 바이너리를 `~/.seogi/bin/seogi`에 복사
- stop.sh가 바이너리를 호출하도록 변경
- 기존 analyzers/session-summary.sh 삭제

### Step 8: 테스트

- `cargo test` — 각 모듈 단위 테스트
  - log_reader: compact/pretty-printed 파싱, 세션 필터링, 빈 파일
  - session_summary: 지표 10개 각각 독립 테스트
  - report: 통계 계산 정확성
  - changelog: 파일 append 동작
